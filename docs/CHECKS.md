...
| 3 | No stock or generated images; empty slots are labelled blocks | pass | 8 images, 0 from other sites, 175 labelled placeholder blocks |
...
| 8 | Every image has alt text; every placeholder block is labelled for a screen reader | pass | 173 labelled blocks; 0 images without alt |
...
| 13 | Every WhatsApp link opens with the message already written | pass | 296 links, 0 without a message |
| 14 | Light theme only; analytics load only once an id is set | pass | 43 KB of CSS read from 1 file(s): no colour-scheme query · analytics on 0 pages, id empty |
...
| 16 | Every internal link and anchor goes somewhere | pass | 954 links across 38 pages, and every same-page anchor has its target |
...
| 19 | Every button says one of two things: talk to her, or pay | pass | 130 buttons, every one from the allowed set |
| 22 | No page shows an unfilled {placeholder} | pass | 38 pages |
| 23 | Every button that takes money reaches a real payment channel | pass | no payment account yet: 0 pay buttons and 4 reserved box(es) standing in their place |
| 24 | Every page that asks for a fee carries the PIN and OTP warning | pass | 2 page(s) ask for money, all 2 carry it |
| 25 | No image carries two width utilities that silently fight each other | pass | 8 image tag(s) across 38 pages, each told its width once |

25 passed, 0 failed, 2 waiting on her material.
