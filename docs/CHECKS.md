...
| 13 | Every WhatsApp link opens with the message already written | pass | 300 links, 0 without a message |
| 14 | Light theme only; analytics load only once an id is set | pass | 45 KB of CSS read from 1 file(s): no colour-scheme query · analytics on 0 pages, id empty |
...
| 19 | Every button says one of two things: talk to her, or pay | pass | 132 buttons, every one from the allowed set |
| 22 | No page shows an unfilled {placeholder} | pass | 38 pages |
| 23 | A reader can always press something and end up somewhere that takes the money | pass | 8 pay button(s), every one to a gateway, a UPI app or WhatsApp |
| 24 | Every page that asks for a fee carries the PIN and OTP warning | pass | 4 page(s) ask for money, all 4 carry it |
