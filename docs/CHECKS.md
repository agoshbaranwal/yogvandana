...
| 8 | Every image has alt text; every placeholder block is labelled for a screen reader | pass | 144 labelled blocks; 0 images without alt |
...
| 14 | Light theme only; analytics load only once an id is set | pass | 35 KB of CSS read from 1 file(s): no colour-scheme query · analytics on 0 pages, id empty |
...
| 16 | Every internal link and anchor goes somewhere | pass | 1098 links across 38 pages, and every same-page anchor has its target |
...
| 19 | Every button says one of two things: talk to her, or pay | pass | 190 buttons, every one from the allowed set |
