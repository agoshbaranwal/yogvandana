...
| 3 | No stock or generated images; empty slots are labelled blocks | pass | 0 images, 0 from other sites, 224 labelled placeholder blocks |
...
| 8 | Every image has alt text; every placeholder block is labelled for a screen reader | pass | 138 labelled blocks; 0 images without alt |
...
| 14 | Light theme only; analytics load only once an id is set | pass | 39 KB of CSS read from 1 file(s): no colour-scheme query · analytics on 0 pages, id empty |
| 15 | Every text colour clears 4.5:1 on its background | pass | 17 pairs measured, the closest being the band's small print at 4.62:1 |
| 16 | Every internal link and anchor goes somewhere | pass | 1092 links across 38 pages, and every same-page anchor has its target |
...
| 19 | Every button says one of two things: talk to her, or pay | pass | 150 buttons, every one from the allowed set |

17 passed, 0 failed, 2 waiting on her material.
