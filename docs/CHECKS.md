...
| 13 | Every WhatsApp link opens with the message already written | pass | 292 links, 0 without a message |
| 14 | Light theme only; analytics load only once an id is set | pass | 46 KB of CSS read from 1 file(s): no colour-scheme query · analytics on 0 pages, id empty |
...
| 19 | Every button says one of two things: talk to her, or pay | pass | 128 buttons, every one from the allowed set |
